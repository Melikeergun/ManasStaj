...
const TemperatureDataScreen = ({ route, navigation }) => {
  const { meter_id, type } = route.params || {};
  console.log('Received params in TemperatureDataScreen:', meter_id, type);
  ...
  useEffect(() => {
    if (meter_id) {
      ...
      console.error('Error: Missing meter_id');
      setLoading(false);
    }
  }, [meter_id]);
  ...
  if (!meter_id) {
    return (
      <View style={styles.container}>
        <Text style={styles.errorText}>Eksik parametreler: meter_id tanımlanmamış</Text>
      </View>
    );
  }
